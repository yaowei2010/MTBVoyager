...
def run_pipeline(newjobID):
    ...
    subfolders = {
        ...
        "CIVIC": ["CIVic.2024.clinicalevidence.csv", "CIVic.assertion.2024.csv"],
        "COSMIC": ["COSMIC_database.tsv"],
        "Mycancergenome": ["MyCancerGenome_Biomarker.json"],
        "oncoKB": [
    ...
    civic_clinical_evidence_path = file_paths['CIVIC'].get('CIVic.2024.clinicalevidence.csv')
    civic_assertion_path = file_paths['CIVIC'].get('CIVic.assertion.2024.csv')

    cosmic_database_path = file_paths['COSMIC'].get('COSMIC_database.tsv')

    mycancergenome_biomarker_path = file_paths['Mycancergenome'].get('MyCancerGenome_Biomarker.json')
    ...
    input_file =vep_annovar_file  # 請更改為你的 CSV 檔案路徑
    ...
    CIVICdb2 = CIVICdb2[['Mark','gene','variant','chromosome','start','end','reference_bases','variant_bases','chromosome2','start2','end2','variant_types','hgvs_descriptions','variant_aliases','clinvar_ids','disease','phenotypes','therapies','evidence_type','evidence_direction','evidence_level','significance','evidence_statement','citation']]
    ...
    def search_civic(gene, variant, tag, CIVICdb1, CIVICdb2):
        # 使用 changeMark 函數生成查詢字串
        data = changeMark(gene, variant, tag)
        print(f"Searching for: {data}")

        # 在 CIVICdb1 中查找
        result_assertion = CIVICdb1[CIVICdb1['Mark'] == data]
        if not result_assertion.empty:
            print("Found in CIVICdb1 (Assertion):")
            print(result_assertion)
            result_assertion = result_assertion[['Mark', 'therapies', 'disease', 'assertion_description']]
            if 'amp_category' in result_assertion.columns:
                result_assertion['Rank'] = result_assertion['amp_category'].map({
                    'Tier I - Level A': 'Tier 1A',
                    'Tier I - Level B': 'Tier 1B',
                    'Tier II - Level C': 'Tier 2C',
                    'Tier II - Level D': 'Tier 2D'
                ...
                result_assertion['Rank'] = result_assertion['Rank'].fillna('Tier 4')
            result_assertion.columns = ['Variant', 'Therapy', 'Disease', 'Detail', 'Rank']
            result_assertion['ClinicalDB'] = 'CIViC-assertion'
            return result_assertion
        else:
            print("Not found in CIVICdb1 (Assertion)")

        # 在 CIVICdb2 中查找
        if 'evidence_level' in CIVICdb2.columns:
            # print(CIVICdb2['evidence_level'])
            # 在 CIVICdb2 中查找
            result_evidence = CIVICdb2[CIVICdb2['Mark'] == data]
            if not result_evidence.empty:
                print("Found in CIVICdb2 (Evidence):")
                if 'evidence_level' in result_evidence.columns:
                    evidence = result_evidence['evidence_level']
                    print("evidence is ")
                    print(evidence)
                    result_evidence = result_evidence[['Mark', 'therapies', 'disease', 'evidence_statement']]
                    # 映射 evidence_level 到 Rank
                    result_evidence['Rank'] = evidence.map({
                        'A': 'Tier 1A',
                        'B': 'Tier 1B',
                        'C': 'Tier 2C',
                        'D': 'Tier 2D',
                    ...
                    result_evidence.columns = ['Variant', 'Therapy', 'Disease', 'Detail', 'Rank']
                    result_evidence['ClinicalDB'] = 'CIViC-evidence'
                    print(result_evidence)
                    return result_evidence
                else:
                    print("evidence_level column not found in filtered CIVICdb2 result")
            else:
                print("Not found in CIVICdb2 (Evidence)")

        else:
            print("evidence_level column not found in CIVICdb2")
    ...
    process_csv(store_file, CIVICdb1, CIVICdb2)
    ...
    for index, row in input_df.iterrows():
        ...
        search_instance = searchMyCancerGenome(gene, data, tag, myCancerGenome)
    ...
    # 保存输出文件
    output_csv_file = r"C:\Users\a5619\OneDrive\桌面\生物及金融大數據實驗室\林醫師VCF團隊\20240820資料\searchCIViC\22W00198_S33_gpu_HF_final_merge_variant_lastest_version1.csv"
    ...
    def searchCGI(query, variantType, gene, phenotype):
        # refine the phenotype if acronyms
        ...
        if variantType == 'Mutation':
            # check query is following the HGVS:
            match = is_hgvs_protein_variant(query)
            if match:
                if query in CGI:
                    cgis = CGI.get(query)
                    # check phenotype
                    phenomatch = [item for item in cgis if item.get('Primary Tumor type full name') == phenotype]
                    if phenomatch and len(cgis) >= 1: # actionable and phenotype is fit
                        for item in phenomatch:
                            gene = item['Gene']
                            drug = item['Drug']
                            ...
                            if approval == 'Approved': # fit phenotype and FDA approved drug
                                tier = 'Tier 1A'
                            else:
                                tier = 'Tier 1B'
                            data = {'code': 200, 'Msg':{"Gene":gene, 'Drug':drug, 'DrugName': drugfullname, 'Approval': approval, 'Evidence': evidence, 'Source': source, 'Phenotype': pheno, 'Tier': tier}}
                            record.append(data)
                    elif len(phenomatch) == 0 and len(cgis) >= 1:
                        for item in cgis:
                            gene = item['Gene']
                            drug = item['Drug']
                            drugfullname = item['Drug full name']
                        ...
                        # Check mutation only: xx oncogenic mutation
                        matching_items = {key: value for key, value in CGI.items() if gene in key}
                        if matching_items:
                            for key, items_list in matching_items.items():
                            ...
                            # Get the data for not searching
                            data = {'code': 500, 'Msg':''}
                            record.append(data)
                else: # not initially in CGI database
                    # Check mutation only: xx oncogenic mutation
                    matching_items = {key: value for key, value in CGI.items() if gene in key}
                    for key, items_list in matching_items.items():
                        for item in items_list:
            ...
            gene1 = query.split('-')[0]
            gene2 = query.split('-')[1]
            queryFusion = query + ' Fusion'
            if query in CGI:
                cgis = CGI.get(queryFusion)
                phenomatch = [item for item in cgis if item.get('Primary Tumor type full name') == phenotype]
                if phenomatch and len(cgis) >= 1: # actionable and phenotype is fit
                    for item in phenomatch:
                        gene = item['Gene']
                    ...
                    # Check Fusion gene only: xxx Fusuion
                    gene1f = gene1 + ' Fusion'
                    gene2f = gene2 + ' Fusion'
                    matching_items1 = {key: value for key, value in CGI.items() if gene1f in key}
                    matching_items2 = {key: value for key, value in CGI.items() if gene2f in key}
                    matching_items1.update(matching_items2)
            ...
            # deletion, amplication
            # Check term
            gene = query.split(' ')[0]
            cnv = query.split(' ')[1]
            if cnv == 'Amplication' or cnv == 'Gain':
                cnv = 'amplication'
            elif cnv == 'Deletion' or cnv == 'Loss':
                cnv = 'deletion'
            else:
                pass
            queryCNV = gene + ' ' + cnv
            if query in CGI:
                cgis = CGI.get(queryCNV)
                phenomatch = [item for item in cgis if item.get('Primary Tumor type full name') == phenotype]
                if phenomatch and len(cgis) >= 1: # actionable and phenotype is fit
                    for item in phenomatch:
                        gene = item['Gene']
    ...
    cgi_results = []
    phenotype = "None"  # 使用指定的 phenotype
...
def pipeline(csv_filename, output_csv_filename, newjobID):
    ...
    data_sources = {
        'CIVIC_result': {
            'key': 'therapies',
            'tier_key': 'evidence_level',
            'database': 'CIVIC',
            'description_key': 'evidence_statement'  # 新增描述鍵
        },
    ...
    def standardize_tier(tier):
        ...
        """
        tier = str(tier).strip()
        return tier_mapping.get(tier, tier)  # 如果找不到對應映射，保持原樣
...
def final_result_somatic(file_path,newjobID):
    ...
    data1 = data[data['Match'].str.contains('CIViC', na=False)]
