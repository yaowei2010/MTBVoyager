...
def delete_job(request):
    if request.method == 'POST':
        try:
            ...
            if job_record:
                resultFile_url = job_record.resultFile_url
                uploadFile_url = job_record.uploadFile_url
                sampleID = job_record.subject_id
                ...
                print("************************")
                print("Result File URL:", resultFile_url)
...
def predicted_suspect_variant(request):
    if request.method == 'POST':
        ...
        parameters['suspect_pheno_variant'].to_csv('predicted_suspect_variant.csv', index=False)

        # Process data
        data = []
        with open('predicted_suspect_variant.csv', mode='r', encoding='utf-8-sig') as original_file:
            reader = csv.DictReader(original_file)
