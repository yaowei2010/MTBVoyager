...
# 如果你的 existJobs 模型不在同檔案，請改正確匯入路徑
from .models import existJobs


def delete_tables_for_job(user_id, newJobID, dsn=None, verbose=True):
    """
    刪除 schema=user_<user_id> 中所有與 newJobID 相關的表。
    先嘗試固定可能出現的名稱，再用 LIKE 搜索補刪。
    回傳實際嘗試刪除的表清單（去重後）。
    """
    ...
    explicit_tables = [
        f"vep_annovar_merge_{newJobID}",
        f"somatic_result_{newJobID}",
        f"{newJobID}_classification",
        f"{newJobID}_somaticResult",
        # 若曾建立過：f"{newJobID}_COSMIC",
    ]
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for t in explicit_tables:
                if verbose:
                    print(f"→ DROP IF EXISTS {schema_name}.{t}")
                cur.execute(
                    sql.SQL('DROP TABLE IF EXISTS {}.{} CASCADE')
                       .format(sql.Identifier(schema_name), sql.Identifier(t))
                )
                dropped.append(f"{schema_name}.{t}")

            # 2) 搜尋該 schema 內所有包含 newJobID 的表（大小寫不敏感）
            cur.execute(
                """
                SELECT table_name
                FROM information_schema.tables
                WHERE table_type='BASE TABLE'
                  ...
                  AND LOWER(table_name) LIKE LOWER(%s)
                """,
                (schema_name, f"%{newJobID}%")
            )
            for (tname,) in cur.fetchall():
                if verbose:
                    print(f"→ DROP (matched) {schema_name}.{tname}")
                cur.execute(
                    sql.SQL('DROP TABLE IF EXISTS {}.{} CASCADE')
                       .format(sql.Identifier(schema_name), sql.Identifier(tname))
                )
                dropped.append(f"{schema_name}.{tname}")

        conn.commit()
        if verbose:
            print(f"✅ 已清理 user_id={user_id} / newJobID={newJobID} 相關資料表")
    except Exception as e:
        if conn:
            conn.rollback()
        if verbose:
...
def safe_rmtree(target_path, must_under="/miRTI/media/patient"):
    ...
    if os.path.isdir(real_target):
        ...
        print(f"🗑️ 已刪除檔案：{real_target}")
    else:
        print(f"ℹ️ 找不到路徑：{real_target}")
...
def delete_job(request):
    ...
    Body: {"newjobid": "<ID>"}
    流程：
      1) 從 existJobs 取出紀錄 → 取得 user_id
      2) 先刪 PostgreSQL 中 schema=user_<user_id> 內與 newJobID 相關的表
      3) 刪除檔案資料夾（優先新結構 /miRTI/media/patient/<user_id>/<newJobID>，再回退舊結構）
      4) 刪除 ORM 紀錄
      5) 若 DB 刪除出錯，回傳 207 並附警告
    """
    if request.method != 'POST':
        return JsonResponse({'error': 'Invalid request method'}, status=405)
    ...
    newJobID = (data.get('newjobid') or '').strip()
    if not newJobID:
        return JsonResponse({'error': 'newjobid is required'}, status=400)
    ...
    # 2) 先刪 PostgreSQL 資料表（以 user_<user_id> schema 為準）
    pg_err = None
    try:
        deleted_tables = delete_tables_for_job(user_id=user_id, newJobID=newJobID, dsn=PG_DSN, verbose=True)
        print("已嘗試刪除表：", deleted_tables)
    except Exception as e:
        pg_err = str(e)  # 不中斷後續檔案/ORM 刪除

    # 3) 刪資料夾：優先新架構 → 舊架構 → 由 uploadFile_url 推斷
    candidates = [
        f"/miRTI/media/patient/{user_id}/{newJobID}",  # 新：按使用者分層
        f"/miRTI/media/patient/{newJobID}",            # 舊：未分層
    ]
    if uploadFile_url:
        candidates.append(os.path.dirname(uploadFile_url))

    deleted_paths = []
    for p in candidates:
        if p and os.path.exists(p):
            safe_rmtree(p, must_under="/miRTI/media/patient")
    ...
    if pg_err:
        resp['postgres_warning'] = pg_err
        return JsonResponse(resp, status=207)  # 部分成功
...
def drug_response_variant(request):
    if request.method == 'POST':
        ...
        csv_filepath = f"/miRTI/media/patient/{newJobID}/result_table/{csv_filename}"
