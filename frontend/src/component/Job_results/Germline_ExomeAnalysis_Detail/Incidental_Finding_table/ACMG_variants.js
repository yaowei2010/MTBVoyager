...
function ACMG_variants({ACMGvariantsData, ACMGSelectionChange, ACMGrowSelectionModel, ACMGsetrowSelectionModel}) {
  ...
  const handleChange = (id) => (event) => {
    ...
    if (ACMGrowSelectionModel.includes(id)) {
      const updatedSelectedRows = ACMGvariantsData
        .filter((row) => ACMGrowSelectionModel.includes(row.id))
        .map((row) => ({
          ...row,
      ...
      ACMGSelectionChange(updatedSelectedRows);
    }
  };
        ...
        return (
          <FormControl sx={{ m: 1, minWidth: 120 }}>
            ...
            <Select
              ...
              autoWidth
              label="選擇"
              disabled={!ACMGrowSelectionModel.includes(params.id)}
            >
  ...
  const handleSelectionChange = (newSelection) => {
    ...
    const selectedRows = ACMGvariantsData
    ...
    ACMGSelectionChange(selectedRows);
    ACMGsetrowSelectionModel(newSelection);
  };

  return (
    <div style={{ height: 600, width: '100%' }}>
      <DataGrid
        rows={ACMGvariantsData}
        columns={columns}
