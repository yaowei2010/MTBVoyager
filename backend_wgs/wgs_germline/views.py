...
SNV_FILES = {
    ...
    "acmg_sf": "acmg_sf",
    "acmg_sf_inheritance": "acmg_sf_inheritance_matched",
    "in_silico": "insilico",
}
...
def _metadata(analysis_id):
    ...
    elif metadata.get("status") == "running" and pid and not _process_is_running(pid):
        metadata["status"] = "failed"
        write_json(directory / "metadata.json", metadata)
...
def _process_is_running(pid):
    """Treat an unreaped zombie as exited so failed jobs do not stay running."""
    try:
...
def job_detail(request, analysis_id):
    ...
    # Keep the nested metadata used by the backend while exposing the compact
    # fields consumed by the React result header.
    safe["subject_id"] = safe["subject"].get("subject_id", "")
    safe["population"] = (safe.get("settings") or {}).get("population", "gnomAD_EAS")
    safe["phenotypes"] = (safe.get("settings") or {}).get("phenotypes", [])
    safe["phenotype_gene_count"] = len(metadata.get("resolved_genes", []))
    safe["log_available"] = (directory / "nextflow.log").exists()
    safe["is_demo"] = bool(metadata.get("is_demo", False))
    safe["demo_notice"] = metadata.get("demo_notice", "")
