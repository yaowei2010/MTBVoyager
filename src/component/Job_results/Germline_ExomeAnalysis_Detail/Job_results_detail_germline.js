  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        const formatData = (data) =>
          data.map((item, index) => ({
            ...
            Pathogenicity: Object.entries(item.Pathogenicity)
              .map(([key, value]) => {
                if (key === 'Summary') {
                  const match = value.match(/\((\d+)\/\d+\)/); // 擷取分子
                  const numerator = match ? match[1] : '0';
                  return `Summary: (${numerator}/8)`; // 固定為 /8
                }
                return `${key}: ${value}`;
              })
              .join('\n'),
            SplicingEffect: Object.entries(item['Splicing effect'])
              .map(([key, value]) => {
                if (key === 'Summary') {
                  ...
                  const numerator = match ? match[1] : '0';
                  return `Summary: (${numerator}/3)`; // 固定為 /3
                }
                return `${key}: ${value}`;
              })
              .join('\n'),
        ...
        const predictedOtheresponse = await axios.post(`${config.rootApiIP}/predicted_other_variant`, {newjobID: newJobID});
        ...
        console.log('otherVariantsResponse',otherVariantsResponse)
